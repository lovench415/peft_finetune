...
def get_tokenizer(dataset_name, tokenizer: str = "pinyin"):
    ...
    tokenizer   - "pinyin" do g2p for only chinese characters, need .txt vocab_file
                - "jamo" decomposes Korean consonants and vowels while simultaneously distinguishing between the initial and final components. It can also process pinyin, english
                - "char" for char-wise tokenizer, need .txt vocab_file
                - "byte" for utf-8 tokenizer
                - "custom" if you're directly passing in a path to the vocab.txt you want to use

    vocab_size  - if use "pinyin", all available pinyin types, common alphabets (also those with accent) and symbols
                - if use "jamo", all available koren consonants and vowels types, alphabets(not with accent) and symbos 
...
def convert_char_to_pinyin_orig(text_list, polyphone=True):
...
# modifited the original convert_char_to_pinyin in order to convert from "hangul" to "jamo" discreting 'cho sung' and 'jong sung'
